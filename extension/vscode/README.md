...
There are **three install routes**, and the third adds no artifact to keep in step.
...
`veridian-cockpit-<version>.vsix` is written to this directory and is **not committed** - it is
generated, like `out/` and like the root `dist/`, and `.gitignore` says so. The name carries the
version because `vsce` takes both it and the name out of the manifest it packages, so the file you
hand to somebody says which extension is inside it. It carries `out/`, `package.json`, the README,
the extension icon and the licence at the extension root; it does not carry `src/`, `scripts/`,
`tsconfig*.json`, `package-lock.json` or `node_modules/`. The supplied Veridian Cockpit artwork is
the extension icon and is also shown in the project README. Those exclusions are not a `.vscodeignore`,

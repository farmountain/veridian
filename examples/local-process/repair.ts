#!/usr/bin/env node
/**
 * The external agent, standing in for a coding agent, in the process demo.
 *
 ...
 *
 * So it *learns why it was invoked by reading a file*, exactly as an external agent must. For this
 * world the report is unusually direct in two ways: a `process.stdout` failure quotes the whole stream
 * the command produced next to the text the contract expected, so a line that arrived on the wrong
 * stream is visible as both readings rather than as a diff; and a `process.contents` or
 * `process.size` failure names the file it read, which is a path the agent can open.
 *
 ...
 * criteria being judged independently. In this table that matters more than usual: D1 moves three
 * criteria, so a repair that fixed everything in one pass would never show a reader what one constant
 * reaches.
 *
 * Usage:
...
async function main(): Promise<number> {
  ...
  if (failing.length === 0) {
    ...
    log(
      unknown.length > 0
        ? `nothing to repair, and cart-build.mjs is not in a recognised state for: ${unknown.join(", ")}`

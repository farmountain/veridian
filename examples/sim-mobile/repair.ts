 ...
 * ## The exit protocol
 *
 * `0` means "I changed something, evaluate again". Any non-zero exit stops the loop. That is why the two
 * "nothing to do" paths below return `1` rather than `0`: a loop that kept iterating on an agent that
 * had nothing left to do would spend its iteration budget proving the same thing, and the ceiling would
 * arrive as `MAX_ITERATIONS` instead of as the success it actually was.
 *
 * ## What it repairs, and what it refuses to do
 *
 * It repairs **the first still-injected defect in criterion order**, one per iteration - AC-005, then
 * AC-007, then AC-013, then AC-015. It does not parse the failure report to decide which one: the report
 * is printed, for the transcript, and the defect table decides. The reason is stated rather than hidden -
 * a demo agent that read the report would be a second implementation of the diagnosis, and the demo
 * would then be proving that a hard-coded table works, which is not the claim. One defect per iteration
 * is also what makes the progression a diagnosis rather than a batch edit: iteration three's report is
 * what iteration two's repair left behind.
 *
 * It verifies each repair before announcing it. A function that edits a file must not report a repair it
 * has not confirmed, and `status` re-reads the text it just wrote rather than trusting the return value
 * of the edit. That matters here for a reason specific to this table: `D3` swaps one letter of a
 * permission key, and a repair that matched the wrong key - of the two this application names, the key
 * and the fully-qualified permission - would leave a grant recorded against a name no criterion asks
 * about while reporting itself fixed.
 */

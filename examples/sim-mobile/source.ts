 ...
 * the tree back (`restoreAll`, here). `examples/shopping-cart/demo.ts` taught this project what a second
 * copy costs: it carried its own replacement, left a defect injected, and still printed "restored the
 * correct app", which is the exact failure `repairOne` was extracted to make impossible.
 *
 * ## Why the file is `provision.mjs` and not something the world generates
 *
 * Everything this world judges - the staged bundle, the manifest, the grants, the deep-link
 * registrations, the notification, the keychain entry, the device's own boot state - is *derived* from
 * this one file on every run, and the sandbox is rebuilt from it before each iteration. A defect
 * injected anywhere else would be erased by the next iteration's own provisioning, and the demo would
 * report a repair the world never had a chance to observe. The editable artifact has to be the input,
 * not the output.
 *
 * ## What "inside the application's tree" means here, and why it is stated rather than assumed
 *
 * The application stages its build context under `app/context/` on this machine, and the bundle it
 * installs lands in the sandbox the adapter resolves. Those are two different directories, and the
 * world's write allowance has to name both - a lesson this repository paid for twice, in
 * `examples/local-process` and again in `examples/sim-container`. Nothing in this module resolves a
 * path for the adapter, though: it reads and writes exactly one file, at one URL derived from
 * `import.meta.url`, which is the same file the adapter is told to run.
 *
 ...
 * and a demo that printed "injected 4 defects" after four no-op writes would be reporting a run's inputs
 * from its own intentions instead of from the tree the run was judged in.
 */
 ...
 * loop that would hang a demo instead of failing it, and the count of defects is the honest upper bound
 * on the number of repairs that can be needed.
 *

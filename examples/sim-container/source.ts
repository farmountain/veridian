 ...
 *
 * Every artifact this world judges - the Dockerfile, the build context, the image, the two containers -
 * is *derived* from this one file on every run, and the sandbox is rebuilt from it before each
 * iteration. So a defect injected anywhere else would be erased by the next iteration's own
 * provisioning, and the demo would report a repair the world never had a chance to observe. The
 * editable artifact has to be the input, not the output.
 *

 ...
 *
 * It repairs **the first still-injected defect in criterion order**, one per iteration - AC-010, then
 * AC-011, then AC-018, then AC-022. It does not parse the failure report to decide which one: the
 ...
 * value of the edit. That matters more here than in a world with one defect per file: `D4`'s block is
 * two lines and names a mount, so a repair that matched the wrong one of the application's two mounts
 * would leave a container that never starts while reporting itself fixed.
 */

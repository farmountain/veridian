#!/usr/bin/env node
/**
 * The external agent, standing in for a coding agent, in the environment-twin demo.
 *
 ...
 * failures are unusually legible for one reason worth stating: the failing criterion is usually about
 * a **line the probe printed**, and a `process.stdout` or `process.contents` failure quotes the whole
 * reading beside the text the contract expected. So the agent is handed the report and the twin, not a
 * diff, and the defect it has to find is visible as two renderings rather than as an inference.
 *
 ...
 * runs that show the criteria being judged independently.
 *
 * Usage:
...
async function main(): Promise<number> {
  ...
  if (failing.length === 0) {
    ...
    log(
      unknown.length > 0
        ? `nothing to repair, and capability-probe.mjs is not in a recognised state for: ${unknown.join(", ")}`

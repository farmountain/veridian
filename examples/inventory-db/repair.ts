#!/usr/bin/env node
/**
 * The external agent, standing in for a coding agent, in the database demo.
 *
 ...
 *
 * So it *learns why it was invoked by reading a file*, exactly as an external agent must - and for a
 * database world the failure report is unusually informative, because `db.value` failures carry both
 * the expected value and the value actually read out of the column. There is no screenshot to fall
 * back on and no need for one.
 *
 * The choice of which defect to fix is deterministic - the first one still present, in criterion
 * order - because a repair that fixed all three at once would skip the intermediate runs that show
 * the criteria being judged independently.
 *
 * Usage:
...
async function main(): Promise<number> {
  ...
  if (failing.length === 0) {
    ...
    log(
      unknown.length > 0
        ? `nothing to repair, and build.mjs is not in a recognised state for: ${unknown.join(", ")}`
  ...
  log(`still injected after this repair: ${String(failing.length - 1)} defect(s); the world is rebuilt`);
  return 0;
}

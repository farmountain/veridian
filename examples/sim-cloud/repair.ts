 ...
 * and it edits the application's own provisioning program with no other channel to the engine - which
 * is the whole separation the product rests on: `Agent = Actor`, `Veridian Environment = World`,
 * `Validator = Judge`. Nothing here decides whether the repair worked; the contract does, on the next
 * iteration, by reading the account again.
 *
 * ## The only channel
 *
 ...
 * repair left behind.
 *
 * One defect per iteration matters more in this world than in the others. The fourth defect changes the
 * *reason* the account refuses a request rather than the answer, so a batch edit would take the run from
 * five failures to `PASS` without ever producing the observation that the outcome was already correct
 * and the rule behind it was not.
 *

 ...
 *
 * It repairs **the first still-injected defect in criterion order**, one per iteration - AC-004, then
 * AC-005, then AC-011, then AC-014. It does not parse the failure report to decide which one: the
 ...
 * value of the edit. That matters here for a reason specific to this table: `D1` and `D2` are two full
 * topic lines that differ only in the topic's name, and the fragment they share - `cleanup: "delete"` -
 * occurs **twice** in the file. An edit that matched the fragment rather than the line would repair the
 * wrong log, and the same criterion that caught the defect would then fail again for a reason the
 * transcript did not describe.
 */

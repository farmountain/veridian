...
The remaining work in this repository is documented across **five** files totalling **216,979 bytes**
(measured: `docs/DIGITAL-TWIN-PLAN.md` 15,044; `docs/DIGITAL-TWIN-DESIGN.md` 26,788;
`docs/ISOLATION-AND-MCP-PLAN.md` 62,054; `docs/INSTRUMENT-AND-PROMPT-PLAN.md` 35,625;
...
| **The `isolation` CI job's execution** | The settlement was reached by reading runs **in order**, and it is the order that decides it rather than any single green one. Run `35958111544` job `isolation substrate` **failed** on a platform-dependent assertion. Run `35958249177` was green **in that job alone**, on a run that was red overall because both `gate` jobs failed - which is why the third observation is the one to read: the run for the phase 08 commit is **success with all nine jobs green**, so the job and the run now agree. **Every run dispatched since agrees the same way**, each `success` with all nine jobs green, and their number is deliberately not written down: a count of runs is the one figure on this row that grows without anyone editing the row. Phase 07 is closed; what remains is keeping it true. |

...
**There is no build step.** Node 22 strips types and executes `.ts` directly, so the source tree is
what runs. Every command below was executed on this machine and is quoted from its real output.
...
it has to be runnable before anything in the repository has been type-checked.

**CI runs the same gates, on both platforms.** [`.github/workflows/ci.yml`](./.github/workflows/ci.yml)
runs `npm run gate` on `ubuntu-latest` and `windows-latest` (both resolving Node from `.nvmrc`), then
the canonical demo on ubuntu, asserts that `--browser none` really exits 2, and uploads `.veridian/`
as an artifact. It is the closest thing to "somebody else installed this" that exists while nobody
else has. The exit-2 step reads `$?` after `set +e` because Actions runs bash with `-e`, which would
abort on the 2 before the assertion could look at it.

## Distribution

**The shipped model is a clone.** There is no build step and no npm package, and that is a decision
with two independent verified causes rather than an unfinished task.
...
`npm link` against one - and cannot work for anyone who installs the package into `node_modules`.
Independently, the schema set resolves through `core/schema/registry.ts` as repo-relative paths
(`schemas/goal.schema.json`) while `nodeIo`'s root defaults to `process.cwd()`, so an installed CLI
would report a missing goal schema that is sitting inside its own package.

Making Veridian installable therefore needs a real build: `tsc` to `dist/*.js` with
`rewriteRelativeImportExtensions`, a published-files allowlist, a copy step for the demo's HTML/CSS,

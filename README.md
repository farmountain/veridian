...
npm run gate              # tsc --noEmit, then the whole test suite. 427 tests, under a second.
...
validators/database/    db.query/value/count/table/column
...
deterministic acceptance criteria + evidence + reset/replay. Two adapters exist: `local-web`, and
`local-db` - the second one being the proof that `EnvironmentAdapter` is a seam rather than a browser
harness with an interface bolted on.
...
what comes after the MVP: the npm and Docker routes, the VS Code Cockpit, the remaining sandbox
worlds, and what each one simulates. Its ordering rule is this project's own - *a phase is built when
it can be proven, not when it can be written* - so a world ships only once its demonstration runs
end to end, not as a placeholder that reports `INCONCLUSIVE` for every criterion.
